...
pub fn grasp_text_array_drop_sides(array: Option<Array<Option<SqlString>>>, drop_on_left: Option<u32>, drop_on_right: Option<u32>) -> Result<Option<Array<Option<SqlString>>>, Box<dyn std::error::Error>> {
    match (array, drop_on_left, drop_on_right) {
        (Some(array1), Some(on_left), Some(on_right)) => {
            let mut array2 = Arc::unwrap_or_clone(array1);
            let right2 = (array2.len() - (on_right as usize).min(array2.len()));
